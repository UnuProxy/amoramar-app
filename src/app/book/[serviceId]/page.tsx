...
export default function DirectBookingPage() {
  ...
  useEffect(() => {
    if (formData.date && service?.id && formData.employeeId) {
  ...
  useEffect(() => {
    ...
    if (bookingStep !== 3 || !stripePublicKey || !walletAmount || !service) {
      setWalletLabel(null);
  ...
  useEffect(() => {
    ...
    setPaymentError(null);
  }, [service?.id, formData.date, formData.time]);
  ...
  const stepValid = useMemo<boolean>(() => {
    if (bookingStep === 1) return !!(formData.name && formData.email && formData.phone && formData.employeeId);
    if (bookingStep === 2) return !!(formData.date && formData.time);
    return true;
  ...
  if (loading || authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-neutral-50">
        <Loading text="Cargando servicio..." />
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-neutral-50 p-4">
        <div className="max-w-md w-full bg-white rounded-[32px] p-12 shadow-xl text-center">
          ...
          <h1 className="text-2xl font-black text-neutral-800 uppercase tracking-tight mb-4">
          ...
          <Link
            href="/"
            className="inline-block px-8 py-4 bg-rose-600 text-white font-bold uppercase tracking-widest rounded-2xl hover:bg-neutral-800 transition-all"
          >
    ...
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 via-white to-rose-50/30 overflow-x-hidden w-full">
      ...
      <main className="pt-20 sm:pt-24 pb-8 sm:pb-12 px-3 sm:px-8">
        <div className="max-w-5xl mx-auto">
          {/* Service Hero Card */}
          <div className="bg-white rounded-[24px] sm:rounded-[40px] shadow-xl sm:shadow-2xl overflow-hidden mb-6 sm:mb-8 border border-neutral-100">
            <div className="bg-gradient-to-r from-neutral-800 via-neutral-900 to-rose-900 p-5 sm:p-8 md:p-12 text-white relative overflow-hidden">
              <div className="absolute top-0 right-0 w-40 sm:w-64 h-40 sm:h-64 bg-rose-600/20 rounded-full -mr-20 sm:-mr-32 -mt-20 sm:-mt-32 blur-3xl" />
              <div className="relative">
                <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] sm:tracking-[0.4em] text-rose-300 mb-2 sm:mb-4">
                  Reserva Directa
                </p>
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black tracking-tight uppercase leading-tight mb-2 sm:mb-4">
                  {service.serviceName}
                </h1>
            ...
            {/* Service Details */}
            <div className="p-8 sm:p-12 grid grid-cols-1 sm:grid-cols-3 gap-6 border-b border-neutral-100">
              <div className="text-center p-6 bg-neutral-50 rounded-[24px]">
                <p className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.3em] mb-2">Precio</p>
                <p className="text-3xl font-black text-rose-600 tracking-tight">{formatCurrency(service.price)}</p>
              </div>
              <div className="text-center p-6 bg-neutral-50 rounded-[24px]">
                <p className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.3em] mb-2">Duración</p>
                <p className="text-3xl font-black text-neutral-800 tracking-tight">{service.duration} <span className="text-lg">min</span></p>
              </div>
              <div className="text-center p-6 bg-neutral-50 rounded-[24px]">
                <p className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.3em] mb-2">Depósito</p>
                <p className="text-3xl font-black text-neutral-800 tracking-tight">{formatCurrency(service.price * 0.5)}</p>
                <p className="text-[10px] text-neutral-400 mt-1">50% para confirmar</p>
              </div>
            </div>
                    ...
                    <div className="text-center py-12">
                      ...
                      <div className="bg-neutral-50 rounded-[24px] p-8 mb-8 text-left">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className="font-bold text-neutral-800">{service.serviceName}</p>
                          </div>
                          <div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        {!user && (
                          <button
                            onClick={() => openAuthModal('signup')}
                            className="px-8 py-4 bg-rose-600 text-white font-bold uppercase tracking-widest rounded-2xl hover:bg-neutral-800 transition-all"
                          >
                    ...
                    <>
                      ...
                      <div className="bg-neutral-50 rounded-[24px] p-8 space-y-4">
                        <div className="flex justify-between items-center pb-4 border-b border-neutral-200">
                          ...
                          <span className="font-bold text-neutral-800">{service.serviceName}</span>
                        </div>
                        <div className="flex justify-between items-center pb-4 border-b border-neutral-200">
